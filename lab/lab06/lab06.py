class Transaction:
    ...
    def report(self):
        ...
        if self.changed():
            "*** YOUR CODE HERE ***"
            msg_ = str(self.before) + '->' + str(self.after)
            # if(self.before < self.after):
            #     _msg = 'increased'
            # else:
            #     _msg = 'decreased'
            _msg = 'increased ' if  (self.before < self.after) else 'decreased '
            msg = _msg + msg_
...
class Account:
    ...
    def deposit(self, amount):
        ...
        ts = Transaction(self.id, before, self.balance)
        self.transactions.append(ts)
    ...
    def withdraw(self, amount):
        ...
        if amount > self.balance:
            ts = Transaction(self.id, self.balance, self.balance)
            self.transactions.append(ts)
...
def make_change(amount, coins):
    ...
    if(result):
        ...
        # If result is None, remove smallest from rest to prevent further use
        if smallest in rest:
            del rest[smallest]  # Remove all coins of the smallest denomination from rest
