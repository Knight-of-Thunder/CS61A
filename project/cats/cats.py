...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    ps = [i for i in paragraphs if select(i)]
    if(k+1 <= len(ps)):
        return ps[k]
    else:
        return ''
...
def about(subject):
    ...
    "*** YOUR CODE HERE ***"
    def exec(paras):
        paras = split(lower(remove_punctuation(paras)))
        for i in subject:
            if i in paras:
                return True
        return False
...
def feline_fixes(typed, source, limit):
    ...
    if(limit - len_diff < 0):   # some optimization
        return len_diff
...
def fastest_words(match):
    ...
    for i in word_indices:
        ...
        for j in player_indices:
            wod_time.append(time(match, j, i))
        mintime = min(wod_time)
        minplayer = wod_time.index(mintime)
        result[minplayer].append(get_word(match, i))
    return result
    # END PROBLEM 10
